...
BEGIN_MESSAGE_MAP(CDynControlDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_ADD, &CDynControlDlg::OnBnClickedBtnAdd)
	ON_BN_CLICKED(IDC_BTN_SUB, &CDynControlDlg::OnBnClickedBtnSub)
END_MESSAGE_MAP()


// CDynControlDlg 消息处理程序
...
void CDynControlDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CDynControlDlg::OnBnClickedBtnAdd()
{
	...
	if(index >= -1 )
	{
		...
		pPictureControl[index]->m_hWnd;//获取HWND句柄
	}
	LeaveCriticalSection(&cs);
}


void CDynControlDlg::OnBnClickedBtnSub()
{
	EnterCriticalSection(&cs);
	
	if(index > -1)
	{
		pPictureControl[index]->DestroyWindow();  
  
		delete pPictureControl[index]; 
		index--;
	}
	LeaveCriticalSection(&cs);
}
